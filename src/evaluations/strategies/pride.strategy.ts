...
@Injectable()
export class PrideStrategy implements ScoringStrategy {
  constructor(
  ...
  private interpretOverall(percentage: number, topDimensions: DimensionScore[]): string {
    ...
    return `المميزات السلوكية البارزة لدى الطفل: ${topNames}`
  }

  private generateRecommendations(topDimensions: DimensionScore[]): string[] {
    const recommendations: string[] = []
    for (const dim of topDimensions) {
      recommendations.push(`التركيز على تطوير مهارة ${dim.name}`)
    }
